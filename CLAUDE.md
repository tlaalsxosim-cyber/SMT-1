...
회전 전체 양 끝이 수십 km씩 벌어질 수 있습니다(실측: 62km까지 벌어진 사례). 전부와 대조해야
회전의 실제 지리적 지름이 좁게 유지됩니다.
...
"거리 계산 단일 진입점"(`distKm`/`simulateTrip`)을 그대로 재사용하므로 원본 로직을 고치면
자동으로 같이 바뀌지만, **회전 재계산의 전제 자체**(예: 2회전 이상 차량의 다음 회전 출발
시각 재계산, `RELOAD_MINUTES` 반영)를 바꾸면 `manual-edit.ts`는 그 부분을 의도적으로
생략하고 있으니 — 사람이 옮긴 회전은 화면에서 "다음 회전 확인 필요"로만 안내하고 자동으로는
따라가지 않습니다. `pipeline/run.ts`를 고칠 때 이 파일도 같은 전제로 맞는지 확인하십시오.
...
**R-06 분할은 같은 차량 안에서만입니다** — 기본은 한 차량에 한 업체 물량 전부입니다.
전 차량의 `최대수량`을 넘는 물량만 `reserveMultiTripSplits`가 회전수 2 이상인 차량 한 대를
찾아 그 차량의 회전에 걸쳐서만 조각을 만들고(`splitVehicleId`), `assignDispatch`의 후보 필터가
`splitVehicleId`가 다른 차량이면 그 조각을 후보에서 뺍니다. 이 필터를 빼면 조각이 다른 차량에
흩어지거나 실을 회전이 안 남아 "분할잔여"가 다시 구조적으로 발생합니다. 맞는 차량이 없으면
R-18처럼 쪼개지 않고 통째로 기타에 남깁니다.
...
R-06 **기본은 한 차량에 한 업체 물량 전부**, 초과 시 회전수 2 이상인 **같은 차량의 회전에만**
분할(다른 차량으로 넘기지 않음), 맞는 차량이 없으면 쪼개지 않고 통째로 기타 (2026-09-18)
