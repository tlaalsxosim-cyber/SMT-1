...
**톤수 제한 표기는 "이하"로 통일돼 있지 않습니다** — `3.5톤이하`(이슬푸드) 말고도 `1톤배차`
(달인식자재), `3.5톤까지`(수일푸드·티비비씨), 접미사 없이 그냥 `2.5톤`만 적힌 경우(문전대박푸드)가
실데이터에 다 있습니다. 현업 확인(2026-09-21): "2.5톤은 2.5톤 이하로 배차하라는 요청" — 즉
납품처명/비고(건)에 톤수가 **적혀 있으면 표현과 무관하게 그 숫자가 상한**입니다.
`TONNAGE_LIMIT_RE`(`constants.ts`)가 접미사를 요구하지 않고 숫자+"톤"만 잡는 이유입니다.
접미사를 다시 요구하도록 좁히면 이 패턴들이 전부 무시됩니다.
...
**권역 우선순위는 하드 필터가 아니라 정렬·점수 가점입니다(요청 2026-09-23)** — `assign.ts`의
`growCluster`가 후보를 붙일 때 이미 담긴 배송지와 같은 권역(`regionKeyOf`, 납품처명 3번째 토큰
정규화)을 1순위로 정렬하고, `scoreCluster`가 조합에 섞인 권역 수만큼 0.15씩 벌점을 줍니다.
권역 간 인접 관계 데이터가 없어 "권역 주변"(2순위)은 거리로 근사합니다 — 같은 권역 후보가
없으면 그냥 거리순으로 넘어갑니다. `growCluster`의 붙이기 순서만 바꿔서는 부족합니다 —
`assignDispatch`가 모든 후보를 씨앗으로 돌려 가장 점수가 낮은 조합을 고르는 바깥 루프가 있어서,
...
**TMAP REST API 운영 키 실호출은 2026-09-22에 확인됐습니다.** Demo Mode를 끄고 실데이터
(`docs/출고등록현황.xlsx` · `docs/차량 톤수.xlsx`)로 배차를 끝까지 돌려, 지오코딩 응답 필드
분기(`newLat`/`lat`)와 `routeOptimization10`의 `B1` 순서가 문서대로 동작함을 확인했습니다 —
지오코딩 65건 · sequential 6건 · 경유지 최적화 8건이 콘솔 오류 없이 끝났고, 지도도
"TMAP 실도로 경로"(점선이 아닌 실선)로 정상 렌더됐습니다. 55곳 중 42곳은 바로 좌표가
확정됐고 13곳은 도로명 불일치/remainder로 **주소확인필요**로 분류됐는데, 이건 에러가 아니라
R-13이 의도한 정상 동작입니다(Demo Mode는 시·군 이름만 맞으면 무조건 성공 처리하므로 이런
불일치가 애초에 드러나지 않습니다).
