 ...
 * 차량 톤수 제한 추출 (R-10)
 *
 * 실데이터에 나오는 표기가 "이하"로 통일되어 있지 않다 — "3.5톤이하"뿐 아니라
 * "1톤배차"(1톤 차량으로 배차), "3.5톤까지", 그리고 접미사 없이 그냥 "2.5톤"만
 * 적힌 경우도 있다. 현업 확인(2026-09-21): "2.5톤은 2.5톤 이하로 배차하라는 요청".
 * 즉 이 필드에 톤수가 적혀 있으면 **표현과 무관하게 그 톤수가 상한**이라는 뜻이므로,
 * 접미사를 요구하지 않고 숫자+"톤" 자체를 잡는다.
 */
export const TONNAGE_LIMIT_RE = /(\d+(?:\.\d+)?)\s*톤/;
 ...
 * 섞여 있다 — N11만 296건, N12만 45건, 둘 다 18건, 둘 다 없음 40건. N11 커버리지가
 * 압도적으로 넓어 기본값으로 삼았지만, 어느 유형이 실제 운영 표준인지는 현업이 확정한
 * 값이 아니다. 특정 품번에 기본 유형 수치가 없으면 `computePalletUsage`가 다른 유형으로
 * 자동 폴백한다.
 */
export const DEFAULT_PALLET_TYPE: "N11" | "N12" = "N11";
